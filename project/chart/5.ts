namespace Lab5 {
  ...
  const tokenPatterns: TokenPattern[] = [
    ...
    //Modify
    new TokenPattern(TokenSort.variable, /^[a-z_][A-Za-z0-9_]*$/)
    //
  ...
  function parseLessThan(tokens: TokenList): ParseResult | null {
    ...
    function parsebiggerthan(token: TokenList): ParseResult | null {
        const parseLeftSubtree = parsePrecedence4(tokens);
        if (pareseLeftSubtree == null) return null;
  ...
  export function desugar(tree: PreAST): AST {
      ...
      //return new StringLeaf("UNIMPLEMENTED"); // replace with your code
    else if (tree instanceof LessThanNode)
