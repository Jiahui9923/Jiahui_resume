namespace Lab5 {
  ...
  const tokenPatterns: TokenPattern[] = [
    ...
    new TokenPattern(TokenSort.variable, /^[a-z_]\w*/)
  ];
  ...
  export function desugar(tree: PreAST): AST {
    if (tree instanceof ValueLeaf || tree instanceof NameLeaf)
      ...
      return new PlusNode(desugar(tree.leftSubtree), tree.rightSubtree);
    else if (tree instanceof TimesNode)
      return new TimesNode(desugar(tree.leftSubtree), tree.rightSubtree);
    else if (tree instanceof AndNode)
      ...
      return new LessThanNode(desugar(tree.leftSubtree), tree.rightSubtree);
    else if (tree instanceof EqualNode)
      return new EqualNode(desugar(tree.leftSubtree), tree.rightSubtree);
    else if (tree instanceof GreaterThanNode)
      return new GreaterThanNode(desugar(tree.leftSubtree), tree.rightSubtree);
    else // if (tree instanceof ConditionalNode)
      return new ConditionalNode(
